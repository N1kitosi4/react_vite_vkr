...
//     const { id, book_title, book_author, book_img, content, created_at, username, genre, average_review_rating, rating } = review;

//     const formattedDate = new Date(created_at).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" });

//     const imageURL = book_img ? `http://127.0.0.1:8000${book_img}` : defaultImg;
//     const navigate = useNavigate();

//     const [userRating, setUserRating] = useState(() => {
//         return localStorage.getItem(`userRating-${id}`) ? Number(localStorage.getItem(`userRating-${id}`)) : null;
//     });
...
//     useEffect(() => {
//         if (current_token) {
//             axios.get(`http://127.0.0.1:8000/reviews/get_rating/${id}`, {
...
//             })
//             .catch(error => console.log("Ошибка при загрузке рейтинга:", error.response?.data || error.message));
//         }
//     }, [id, current_token]);

//     const handleRating = (newRating) => {
//         axios.post(
//             `http://127.0.0.1:8000/reviews/rate/${id}`,
//             { rating: newRating },
//             {
//                 headers: {
...
//             localStorage.setItem(`userRating-${id}`, response.data.user_rating); // Сохраняем в localStorage
//         })
//         .catch(error => console.log("Ошибка при выставлении рейтинга:", error.response?.data || error.message));
//     };

//     return (
//         <li className="review">
//             <img src={imageURL} alt="review img" className="review__img" onClick={() => navigate(`/review/${id}`)} />
...
//             <p className="review__book-rating">Рейтинг книги: ★{(rating || 0).toFixed(1)}</p>
//             <p className="review__rating">Средний рейтинг отзыва: ★{avgRating.toFixed(1)}</p>

//             {/* Блок с выставлением оценки */}
...
const HomeReview = ({ review }) => {
  ...
  useEffect(() => {
    if (current_token) {
      ...
      axios
        .get(`http://127.0.0.1:8000/reviews/get_rating/${id}`, {
          headers: { Authorization: `Bearer ${current_token}` },
        })
        .then((response) => {
          setUserRating(response.data.user_rating);
